...
// end include: node_shell_read.js
  ...
  if (!globalThis.WebAssembly) {
    eval(fs.readFileSync(locateFile('DOD_engine_web.wasm.js'))+'');
  }
...
function findWasmBinary() {
  return locateFile('DOD_engine_web.wasm');
}
...
function render_to_canvas(buffer_ptr,width,height) { const canvas = document.getElementById('canvas'); if (!canvas) return; const ctx = canvas.getContext('2d'); if (canvas.width !== width || canvas.height !== height) { canvas.width = width; canvas.height = height; } const imgData = ctx.createImageData(width, height); const src = Module.HEAPU8.subarray(buffer_ptr, buffer_ptr + (width * height * 4)); imgData.data.set(src); for (let i = 0; i < imgData.data.length; i += 4) { imgData.data[i + 3] = imgData.data[i]; imgData.data[i] = imgData.data[i+2]; imgData.data[i + 2] = imgData.data[i + 3]; imgData.data[i + 3] = 255; } ctx.putImageData(imgData, 0, 0); }
