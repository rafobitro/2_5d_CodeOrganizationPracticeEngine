...
//    bmp_loader(textures.data[9], "Wooden_Floor_Horizontal_64x64.bmp");
    ...
    bmp_loader(textures.data[9], "Enemy_64x64.bmp");
    
    
    textures.count = 10;

    export_assets(textures, "assets");
}
...
void export_assets(Textures& textures, std::string path) {
    std::string fullpath = std::string(ASSETS_PATH) + path +".h";
    std::ofstream out(fullpath);
    std::string fullpath2 = std::string(ASSETS_PATH) + path +"_data.h";
    ...
    out << "uint32_t data[" << textures.count << "][" << TEXTURE_PIXELS << "]={\n";
    for (int i = 0;i < textures.count;i++) {
        out << "{  ";
        for (int j = 0;j < TEXTURE_PIXELS;j++) {
            out << " " << textures.data[i][j] << ",";
